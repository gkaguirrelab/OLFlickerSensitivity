function [subject,subjectDataDir,saveFileName] = OLGetSubjectDataDirMR(protocolDataDir,protocolList,protocolIndex)
...
while true
	...
	elseif any(subjectIndex == 1:length(availableSubjects))
		...
		subjectDataDir = sprintf('%s/%s/%s/%s', protocolDataDir, ...
			subject, datestr(now, 'mmddyy'), 'Stimuli');
...
saveFileName = sprintf('%s/%s-%s-%02d.mat', subjectDataDir, ...
