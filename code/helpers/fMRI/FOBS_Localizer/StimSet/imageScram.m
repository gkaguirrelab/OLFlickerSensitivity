function imageScram(folder1,folder2,n,neib,fmt,gridScram,fcolor,gs,sufs,folder3,fftstd)
% IMAGESCRAM.M
...
for i=4:length(d),
	...
	if neib>=5,
		...
		if sx/n~=fix(sx/n),error(sprintf('Dimensions do not agree with divisions for %s',fname)),end
		if sy/n~=fix(sy/n),error(sprintf('Dimensions do not agree with divisions for %s',fname)),end
		bx=sx/n;by=sy/n;
