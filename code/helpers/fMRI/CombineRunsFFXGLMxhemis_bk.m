function glmDirs = CombineRunsFFXGLMxhemis(BASE_PATH, FWHM, SUBJECT_FSL, BOLD_DIR, COMBINED_DIR, INPUT_FILE_PREFIX, FEAT_DIRS, nRuns, COPES, VARCOPES, nCopes)
...
for j = 1:nRuns % Assemble the inputs
    for h = 1:length(hemis)
        theInputs = [theInputs, ' --is ' fullfile(BASE_PATH, SUBJECT_FSL, BOLD_DIR, FEAT_DIRS{j}, ['mean_func.diffeo.' hemis{h} '.sym.mgh '])];
...
for i = 1:nCopes
    ...
    for j = 1:nRuns % Assemble the inputs
        for h = 1:length(hemis)
            theInputs = [theInputs, ' --is ' fullfile(BASE_PATH, SUBJECT_FSL, BOLD_DIR, FEAT_DIRS{j}, INPUT_FILE_PREFIX, ['cope' num2str(i) '.diffeo.' hemis{h} '.sym.mgh '])];
            theSubjects = [theSubjects '--s fsaverage_sym '];
        end
    end
    commandc = ['mris_preproc --fwhm ' FWHM ' --target fsaverage_sym --out ' outFile ' --hemi lh ' theSubjects theInputs];
    ...
    for j = 1:nRuns % Assemble the inputs
        for h = 1:length(hemis)
            theInputs = [theInputs, ' --is ' fullfile(BASE_PATH, SUBJECT_FSL, BOLD_DIR, FEAT_DIRS{j}, INPUT_FILE_PREFIX, ['varcope' num2str(i) '.diffeo.' hemis{h} '.sym.mgh '])];
            theSubjects = [theSubjects '--s fsaverage_sym '];
        end
    end
    commandc = ['mris_preproc --fwhm ' FWHM ' --target fsaverage_sym --out ' outFile ' --hemi lh ' theSubjects theInputs];
    ...
    mkdir(glmDir);
    commandc = ['mri_glmfit --fwhm ' FWHM ' --var-fwhm ' FWHM ' --y  ' inputXRun ' --yffxvar ' inputXRunVar ' --ffxdof ' num2str(DOF)  ...
    ...
    % Do the CWP
    copyfile(inputXRun, glmDir);
    cd(glmDir);
    system('mri_glmfit-sim --glmdir .  --cache 3 abs --cwp 0.01');
end