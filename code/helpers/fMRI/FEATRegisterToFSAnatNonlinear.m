function FEATRegisterToFSAnatNonlinear(session_dir, subjID)

SUBJECTS_DIR = getenv('SUBJECTS_DIR');
FSL_DIR = getenv('FSL_DIR');
stat_files = {'cope' 'varcope'};
...
for r = 1:nruns
    ...
    % Apply the affine transforms and the non-linear warps to write the final masked registered slab image (masked-example_func-2-FS-Anat-Diffeo.nii.gz)
    [s, o] = system(['WarpImageMultiTransform 3 masked-example_func-2-FS-Anat.nii.gz masked-example_func-2-FS-Anat-Diffeo.nii.gz -R slab-brain.nii.gz abWarp.nii.gz abAffine.txt']);
    disp(o);
    ...
    for s = 1:length(stat_files)
        ...
        for n = 1:nfiles
            ...
            [s, o] = system(['WarpImageMultiTransform 3 ' fullfile(session_dir, d{r}, 'stats', [ft '-2-FS-Anat.nii.gz']) ' ' fullfile(session_dir, d{r}, 'stats', [ft '-2-FS-Anat-Diffeo.nii.gz'])  ' -R slab-brain.nii.gz abWarp.nii.gz abAffine.txt']);
            disp(o);
            ...
            for h = 1:length(hemi)
                ...
                %% Unsmoothed - native
                [s,o] = system(['mri_vol2surf --regheader ' subjID ' --src ' fullfile(session_dir, d{r}, 'stats', [ft '-2-FS-Anat-Diffeo.nii.gz']) ' --hemi ' hemi{h} ...
                    ' --out ' fullfile(session_dir, d{r}, 'stats', [ft '.diffeo.' hemi{h} '.mgh']) ' --projfrac 0']);
                disp(o);
                ...
                if strcmp(hemi{h}, 'rh')
                    [s,o] = system(['mri_surf2surf --hemi lh --trgsurfreg sphere.reg --srcsurfreg fsaverage_sym.sphere.reg --srcsubject ' subjID '/xhemi --srcsurfval ' ...
                        fullfile(session_dir, d{r}, 'stats', [ft '.diffeo.' hemi{h} '.mgh']) ' --trgsubject fsaverage_sym --trgsurfval ' ...
                        fullfile(session_dir, d{r}, 'stats', [ft '.diffeo.' hemi{h} '.sym.mgh']) ]);
                    disp(o);
                else
    ...
    for n = 1:length(f)
        ...
        [s, o] = system(['WarpImageMultiTransform 3 ' fullfile(session_dir, d{r}, [ft '-2-FS-Anat.nii.gz']) ' ' fullfile(session_dir, d{r}, [ft '-2-FS-Anat-Diffeo.nii.gz'])  ' -R slab-brain.nii.gz abWarp.nii.gz abAffine.txt']);
        disp(o);
        ...
        for h = 1:length(hemi)
            %Project cope files to surface, for each hemisphere
            % Unsmoothed - native
            [s,o] = system(['mri_vol2surf --regheader ' subjID ' --src ' fullfile(session_dir, d{r}, [ft '-2-FS-Anat-Diffeo.nii.gz']) ' --hemi ' hemi{h} ...
                ' --out ' fullfile(session_dir, d{r}, [ft '.diffeo.' hemi{h} '.mgh']) ' --projfrac 0']);
            disp(o);
            % Unsmooth - fsaverage_sym
            if strcmp(hemi{h}, 'rh')
                [s,o] = system(['mri_surf2surf --hemi lh --trgsurfreg sphere.reg --srcsurfreg fsaverage_sym.sphere.reg --srcsubject ' subjID '/xhemi --srcsurfval ' ...
                    fullfile(session_dir, d{r}, [ft '.diffeo.' hemi{h} '.mgh']) ' --trgsubject fsaverage_sym --trgsurfval ' ...
                    fullfile(session_dir, d{r}, [ft '.diffeo.' hemi{h} '.sym.mgh']) ]);
                disp(o);
            else
                [s,o] = system(['mri_surf2surf --hemi lh --trgsurfreg sphere.reg --srcsurfreg fsaverage_sym.sphere.reg --srcsubject ' subjID ' --srcsurfval ' ...
                    fullfile(session_dir, d{r}, [ft '.diffeo.' hemi{h} '.mgh']) ' --trgsubject fsaverage_sym --trgsurfval ' ...
...
%         %disp(o);
%         
%         [s, o] = system(['WarpTimeSeriesImageMultiTransform 4 ' fullfile(session_dir, d{r}, [ft '-2-FS-Anat.nii.gz']) ' ' fullfile(session_dir, d{r}, [ft '-2-FS-Anat-Diffeo.nii.gz'])  ' -R slab-brain.nii.gz abWarp.nii.gz abAffine.txt']);
%         disp(o);
%         
%         %move to surface
%         for h = 1:length(hemi)
