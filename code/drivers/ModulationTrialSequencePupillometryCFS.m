...
% communications.
matlabUDP('close');
matlabUDP('open', params.macWinHostIP, params.winHostIP, params.udpPort1);

%%* OPEN UDP
if exist('instrfindall') && ~isempty(instrfindall)
    fclose(instrfindall);
end
udpLaptop = udp(params.cfsHostIP, params.udpPort2, 'LocalPort', params.udpPort2);
fopen(udpLaptop);

%%* CONFIRM UDP
fprintf('Waiting to confirm UDP with Laptop. \n');
fprintf(udpLaptop, '1');
while ~udpLaptop.bytesAvailable
    continue;
end
if strtrim(fscanf(udpLaptop)) == '1'
    fprintf('UDP Connection with Laptop established. \n');
    fprintf('You may now start the trial. \n');
else
    fprintf('Error in establishing UDP Connection with Laptop. Retry. \n')
end
flushinput(udpLaptop);
flushoutput(udpLaptop);

%% Run the trial loop.
params = trialLoop(params, block, exp, udpLaptop);

% Send termination signal to the laptop
fprintf(udpLaptop,'z');

% Here, we end the experiment
...
%       - trialLoop(...)
    ...
    function params = trialLoop(params, block, exp, udpLaptop)
        ...
        for trial = 1:params.nTrials
            ...
            %%* SEND CFS start
            if params.cfsOn(trial) == 0 %static
                fprintf(udpLaptop,'a');
            else %CFS
                fprintf(udpLaptop,'b');
            end
            fprintf('> Sent CFS signal to laptop. \n');
            ...
            %%* SEND CFS STOP
            fprintf(udpLaptop,'c');
            fprintf('Sent signal to end trial. \n');
            if strtrim(fscanf(udpLaptop)) == '1'
                fprintf('> Connection with Laptop remains established. \n');
            else
                fprintf('> Confirmation from laptop not received. \n');
            end
            ...
            if (offline == true);
                ...
                dataStruct(trial).phaseCarrier = block(trial).carrierPhaseDeg;
                dataStruct(trial).direction = block(trial).direction;
                % Save out CFS information
                if isfield(params, 'cfsOn')
                    if params.cfsOn(trial)
                        dataStruct(trial).direction = [dataStruct(trial).direction '_CFS'];
                    end
                end
                dataStruct(trial).contrastRelMax = block(trial).contrastRelMax;
            end
