...
for k = 1:params.nTrials
    ...
    trialPrimary = bgPrimary + params.trialSeqLMSContrast(k)*cacheData{2}.data(params.observerAgeInYears).differencePrimary + params.trialSeqMelContrast(k)*cacheData{3}.data(params.observerAgeInYears).differencePrimary;
    ...
    for j = 1:nWindowed
        primaries(:, j) = bgPrimary +powerLevels(j)*((params.trialSeqLMSContrast(k)*cacheData{2}.data(params.observerAgeInYears).differencePrimary ...
            + params.trialSeqMelContrast(k)*cacheData{3}.data(params.observerAgeInYears).differencePrimary)) ...
...
%       - trialLoop(...)
    ...
    function params = trialLoop(params, exp)
        ...
        for trial = 1:params.nTrials
            ...
            % The delay to enter and exit the 'startDataStreaming...' function is ~60 ms.
            
            labjackOBJ.startDataStreamingForSpecifiedDuration(params.trialDuration);
            EMG_time = labjackOBJ.timeAxis;
            EMG_data = labjackOBJ.data;
            ...
            dataStruct(trial).time = EMG_time;
            dataStruct(trial).emgVoltage = EMG_data;
