...
for i = 1:params.nTrials
    ...
    % Repmat so that we can have seemless looping.
    block(i).data.starts = repmat(block(i).data.starts, params.nRepeats, 1);
    block(i).data.stops = repmat(block(i).data.stops, params.nRepeats, 1);
    block(i).data.powerLevels = repmat(block(i).data.powerLevels, 1, params.nRepeats);
    ...
    if ~isfield(block(i).data, 'startsBG')
...
%       - trialLoop(...)
    ...
    function params = trialLoop(params, exp)
        ...
        for trial = 1:params.nTrials
            ...
            %    pause;
            %end
            fprintf('* Start trial %i/%i - %s, Hz.\n', trial, params.nTrials, block(trial).direction);
