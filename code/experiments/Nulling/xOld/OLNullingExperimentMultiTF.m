...
%               2. Subjects reiterate through these three nulling
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Set some parameters
params.nullingFrequency = 1;        % Hz
params.pulsePre = 1;                % seconds
params.nullingDuration = 2;
...
params.nullingStartWeights = {[-0.35, -0.48, 0], [0, 0.42, 0]};
...
params.initialDeflectionWeights = {[(1-params.nullingStartWeights{1}(1)), 0, 0], [0, -0.4, 0]};

% Define the directions which are to be nulled.
params.nullingDirections = {'MelanopsinDirectedLegacy'};
...
params.nullingLuminanceFrequency = {[40], []};
...
params.maxContrastPrimary = 0.4;    % 40% for both MelanopsinDirected and LMSDirected stimuli
params.maxContrastLMS = 0.4;        % 40% for LMS
params.maxContrastLMinusM = 0.12;   % 12% for L-M
params.maxContrastS = 0.4;          % 40% for S
...
calibrationType = 'BoxAShortCableBEyePiece2_ND06';
...
for r = 1:length(params.modulationArms)
    ...
    for p = 1:length(params.nullingDirections)
        ...
        nulling{p, r}.primaryNulling = cacheTmp.data(observerAge).differencePrimary;
        nulling{p, r}.primaryLMinusM = cacheLMinusM.data(observerAge).differencePrimary;
        nulling{p, r}.primaryS = cacheSDirected.data(observerAge).differencePrimary;
        nulling{p, r}.primaryLMS = cacheLMSDirected.data(observerAge).differencePrimary;
        ...
        while NotDoneNulling
            ...
            %% Second phase, L-M
            system('say match the color green to red.');
            %sound(yStart, fs);
            ...
            while keepRunning
                ...
                % Show the pulse
                %sound(yHint, fs);
                for j = 1:params.nullingDuration*params.nullingFrequency;
                    ol.setMirrors(backgroundStarts, backgroundStops);
        ...
        nulling{p, r}.observerAge = observerAge;
        nulling{p, r}.observerID = observerID;
        nulling{p, r}.params = params;
        toc; fprintf('\n')
        
        T_receptors = cacheMelanopsin.data(observerAge).describe.T_receptors(1:5, :);
        photoreceptorClasses = cacheMelanopsin.data(observerAge).describe.photoreceptors;
...
cache.cacheMelanopsinLegacy = cacheMelanopsinLegacy;
