function OLNullingExperimentMultiTF_Test(observerID)

saveOutFile = [observerID '_nulling'];
outPath = '/Users/Shared/Matlab/Experiments/OneLight/OLFlickerSensitivity/data/Nulling';
load(fullfile(outPath, saveOutFile));

backgroundPrimary = nulling{1, 1}.backgroundPrimary;
system('say Now let us see if the nulls hold');
dt = 1/256;
[bgStarts bgStops]= OLSettingsToStartsStops(cal, OLPrimaryToSettings(cal, backgroundPrimary));

paramsValues = [0.5 1 2 8 16 32];
...
for k = 1:nParamsVals
    ...
    for i = 1:length(t0)
        primaryVals1(:, i) = backgroundPrimary+sin(2*pi*theFreq*t0(i))*nullingaverages{1}.differencePrimary;
...
system('say Ready? Press any key.')
pause;
